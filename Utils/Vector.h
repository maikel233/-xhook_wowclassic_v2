...
public:
	...
	// New function to validate the Vector2 instance
	bool isValid() const
	{
		return !isnan(x) && !isnan(y); // Ensure x and y are not NaN
	}

	// New function to check if both x and y are NULL or invalid
	bool isInvalid() const
	{
		return (x == NULL && y == NULL) || !isValid(); // Check if both are NULL or invalid
	}
};
